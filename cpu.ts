...
const node_id = (data.nodes.find((n, i) => {
...
let sum = 0;
let start = 0;
for (; start < data.samples.length && nodes[data.samples[start]].id !== node_id; ++start) {
}
console.log("found start", start);
for (let i = start; i < data.samples.length && data.samples[i] === node_id; ++i) {
